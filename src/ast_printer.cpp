...
static void dump_type(const ast::TypeAnnot &type, std::ostream &os);
static void dump_expr(const ast::Expr &expr, std::ostream &os);
static void dump_stmt(const ast::Stmt &stmt, std::ostream &os);
static void dump_block(const ast::Block &block, std::ostream &os);
static void dump_pattern(const ast::Pattern &pat, std::ostream &os);
...
static void dump_pattern(const ast::Pattern &pat, std::ostream &os) {
  std::visit(
      [&](auto &&arg) {
        using T = std::decay_t<decltype(arg)>;
        if constexpr (std::is_same_v<T, ast::VariantPattern>) {
          indent(os);
          ...
          print_span(os, arg.span);
          os << "\n";
        } else if constexpr (std::is_same_v<T, ast::IdentPattern>) {
          indent(os);
...
static void dump_fn(const ast::FnDecl &fn, std::ostream &os) {
  indent(os);
...
static void dump_decl(const ast::Decl &decl, std::ostream &os) {
  std::visit(
      [&](auto &&arg) {
        using T = std::decay_t<decltype(arg)>;
        if constexpr (std::is_same_v<T, ast::FnDecl>) {
          dump_fn(arg, os);
          ...
          for (auto &m : arg.methods)
            dump_fn(m, os);
          indent_level--;
        } else if constexpr (std::is_same_v<T, ast::InterfaceDecl>) {
          indent(os);
        ...
        } else if constexpr (std::is_same_v<T, ast::MacroDecl>) {
          indent(os);
...
void dump_ast(const ast::Program &program, std::ostream &os) {
