...
optional<ast::Decl> Parser::parse_declaration() {
  ast::Decl decl;
  ...
  if (check(TokenType::KW_COMPTIME)) {
    ...
    decl.value.emplace<ast::ComptimeStmt>(
        std::move(get<ast::ComptimeStmt>(cs_stmt->value.value)));
    return decl;
  }
...
optional<ast::FnDecl> Parser::parse_fn_decl() {
  size_t start = current_pos;
  ...
  if (match(TokenType::ARROW)) {
    ...
    if (!rt)
      return nullopt;
    fn.return_type = make_unique<ast::TypeAnnot>(std::move(rt->value));
  }

  // optional where clause
  if (check(TokenType::KW_WHERE)) {
    advance();
    vector<ast::WhereConstraint> constraints;
...
optional<ast::StructDecl> Parser::parse_struct_decl() {
  size_t start = current_pos;
...
optional<ast::UnionDecl> Parser::parse_union_decl() {
  size_t start = current_pos;
...
optional<ast::InterfaceDecl> Parser::parse_interface_decl() {
  size_t start = current_pos;
...
optional<ast::ExternDecl> Parser::parse_extern_decl() {
  size_t start = current_pos;
  ...
  if (match(TokenType::ARROW)) {
    ...
    if (!rt)
      return nullopt;
    decl.return_type = make_unique<ast::TypeAnnot>(std::move(rt->value));
  }
...
optional<ast::UseDecl> Parser::parse_use_decl() {
  size_t start = current_pos;
...
optional<ast::MacroDecl> Parser::parse_macro_decl() {
  size_t start = current_pos;
...
optional<Spanned<ast::PrimitiveType>> Parser::parse_primitive_type() {
...
optional<Spanned<ast::InterfaceTypeAnnot>> Parser::parse_interface_type() {
  size_t start = current_pos;
  consume(TokenType::KW_INTERFACE, "'interface'");
  if (has_errors())
    return nullopt;
...
optional<Spanned<ast::TypeAnnot>> Parser::parse_named_type() {
  size_t start = current_pos;
  ...
  if (match(TokenType::LBRACKET)) {
    while (!check(TokenType::RBRACKET) && !is_at_end()) {
      ...
      named.generic_args.push_back(
          make_unique<ast::TypeAnnot>(std::move(t->value)));
      if (!match(TokenType::COMMA))
        break;
    }
    consume(TokenType::RBRACKET, "']'");
...
optional<Spanned<ast::TypeAnnot>> Parser::parse_type() {
  return parse_type_with_prefix_operators();
}

optional<Spanned<ast::TypeAnnot>> Parser::parse_type_with_prefix_operators() {
  size_t start = current_pos;
  ...
  if (match(TokenType::STAR)) {
    ...
    ptr.inner = make_unique<ast::TypeAnnot>(std::move(inner->value));
  ...
  if (match(TokenType::LBRACKET)) {
    if (match(TokenType::RBRACKET)) {
      ...
      slice.inner = make_unique<ast::TypeAnnot>(std::move(inner->value));
    ...
    arr.size = make_unique<ast::Expr>(std::move(size_expr->value));
    arr.inner = make_unique<ast::TypeAnnot>(std::move(inner->value));
  ...
  if (match(TokenType::QUESTION)) {
    ...
    opt.inner = make_unique<ast::TypeAnnot>(std::move(inner->value));
...
optional<ast::Param> Parser::parse_param() {
  size_t start = current_pos;
  ...
  p.type = make_unique<ast::TypeAnnot>(std::move(t->value));
  p.span = get_span_for(start);
  return p;
}

optional<ast::GenericParam> Parser::parse_generic_param() {
  size_t start = current_pos;
...
optional<ast::WhereConstraint> Parser::parse_where_constraint() {
  size_t start = current_pos;
...
optional<ast::ExternParam> Parser::parse_extern_param() {
  size_t start = current_pos;
  auto t = parse_type();
  if (!t)
    return nullopt;
  ...
  ast::ExternParam ep;
  ep.type = make_unique<ast::TypeAnnot>(std::move(t->value));
  ep.span = get_span_for(start);
  return ep;
}

optional<ast::MacroParam> Parser::parse_macro_param() {
  size_t start = current_pos;
...
optional<ast::InterfaceMethod> Parser::parse_interface_method() {
  size_t start = current_pos;
  consume(TokenType::KW_FN, "'fn'");
  if (has_errors())
    return nullopt;
  ...
  if (match(TokenType::ARROW)) {
    ...
    if (!rt)
      return nullopt;
    method.return_type = make_unique<ast::TypeAnnot>(std::move(rt->value));
  }
...
optional<ast::FieldDecl> Parser::parse_field_decl() {
  size_t start = current_pos;
  ...
  fd.type = make_unique<ast::TypeAnnot>(std::move(t->value));
  fd.span = get_span_for(start);
  return fd;
}

optional<ast::UnionVariant> Parser::parse_variant_decl() {
  size_t start = current_pos;
  ...
  if (match(TokenType::COLON)) {
    ...
    if (!t)
      return nullopt;
    v.type = make_unique<ast::TypeAnnot>(std::move(t->value));
  }
